...
// *** LEXER TABLE START ***
    ...
    fn s0(&mut self, mut ctx: Context) -> Option<<Self as Iterator>::Item> {
        ...
        if ch == 0x29 { self.index += 1; return self.s1(ctx); }
        if ch == 0x26 { self.index += 1; return self.s2(ctx); }
        if ch == 0x27 { self.index += 1; return self.s3(ctx); }
        if ch == 0x2b { self.index += 1; return self.s7(ctx); }
        if ch == 0x2d { self.index += 1; return self.s8(ctx); }
        if ch == 0x7c { self.index += 1; return self.s9(ctx); }
        if ch == 0x3f { self.index += 1; return self.s10(ctx); }
        if ch == 0x21 { self.index += 1; return self.s11(ctx); }
        if ch == 0x2a { self.index += 1; return self.s12(ctx); }
        if ch == 0x09 ||
           ch == 0x0a ||
           ch == 0x0d ||
           ch == 0x20 { self.index += 1; return self.s13(ctx); }
        if ch == 0x28 { self.index += 1; return self.s14(ctx); }
        if ch == 0x2e { self.index += 1; return self.s15(ctx); }
        if ch == 0x22 { self.index += 1; return self.s17(ctx); }
        self.begin()
    }
    ...
    fn s3(&mut self, mut ctx: Context) -> Option<<Self as Iterator>::Item> {
        ...
        if ch == 0x09 ||
           ...
           (0x23 <= ch && ch <= 0x5b) ||
           (0x5d <= ch && ch <= 0x7e) ||
           (0xa0 <= ch && ch <= 0xff) { self.index += 1; return self.s4(ctx); }
        if ch == 0x5c { self.index += 1; return self.s6(ctx); }
        self.sink(ctx)
    }
    ...
    fn s4(&mut self, mut ctx: Context) -> Option<<Self as Iterator>::Item> {
        if self.index >= self.input.len() { return self.sink(ctx); }
        let ch = self.input[self.index];
        if ch == 0x27 { self.index += 1; return self.s5(ctx); }
        self.sink(ctx)
    }
    ...
    fn s6(&mut self, mut ctx: Context) -> Option<<Self as Iterator>::Item> {
        ...
        let ch = self.input[self.index];
        if ch == 0x22 ||
           ch == 0x5c { self.index += 1; return self.s4(ctx); }
        self.sink(ctx)
    }
    ...
    fn s17(&mut self, mut ctx: Context) -> Option<<Self as Iterator>::Item> {
        ...
        if ch == 0x09 ||
           ...
           (0x23 <= ch && ch <= 0x5b) ||
           (0x5d <= ch && ch <= 0x7e) ||
           (0xa0 <= ch && ch <= 0xff) { self.index += 1; return self.s18(ctx); }
        if ch == 0x5c { self.index += 1; return self.s19(ctx); }
        self.sink(ctx)
    }
    ...
    fn s18(&mut self, mut ctx: Context) -> Option<<Self as Iterator>::Item> {
        ...
        if ch == 0x09 ||
           ...
           (0xa0 <= ch && ch <= 0xff) { self.index += 1; return self.s18(ctx); }
        if ch == 0x5c { self.index += 1; return self.s19(ctx); }
        if ch == 0x22 { self.index += 1; return self.s20(ctx); }
        self.sink(ctx)
    }
...
pub fn parse<I: AsRef<[u8]> + ?Sized>(input: &I) -> Result<Variable, ParseError> {
    let mut parse = Parse { input: scan(input), next_token: None, trace: Vec::new() };
    parse.update()?;
    let result = parse.s0();
    if let Err(_) = result { println!("{:?}", parse.trace); }
    Ok(parse.s0()?.0)
}
