...
// *** LEXER TABLE START ***
    ...
    fn s0(&mut self, mut ctx: Context) -> Option<<Self as Iterator>::Item> {
        ...
        if ch == 0x09 ||
           ch == 0x0a ||
           ch == 0x0d ||
           ch == 0x20 { self.index += 1; return self.s2(ctx); }
        if ch == 0x26 { self.index += 1; return self.s3(ctx); }
        if ch == 0x3f { self.index += 1; return self.s4(ctx); }
        if ch == 0x7c { self.index += 1; return self.s5(ctx); }
        if ch == 0x22 { self.index += 1; return self.s6(ctx); }
        if ch == 0x2d { self.index += 1; return self.s10(ctx); }
        if ch == 0x27 { self.index += 1; return self.s11(ctx); }
        if ch == 0x2b { self.index += 1; return self.s15(ctx); }
        if ch == 0x21 { self.index += 1; return self.s16(ctx); }
        if ch == 0x29 { self.index += 1; return self.s17(ctx); }
        if ch == 0x2e { self.index += 1; return self.s18(ctx); }
    ...
    fn s18(&mut self, mut ctx: Context) -> Option<<Self as Iterator>::Item> {
        if self.index >= self.input.len() { return self.sink(ctx); }
        let ch = self.input[self.index];
        if ch == 0x2e { self.index += 1; return self.s19(ctx); }
        self.sink(ctx)
    }
