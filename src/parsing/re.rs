...
// *** LEXER TABLE START ***
    ...
    fn s0(&mut self, mut ctx: Context) -> Option<<Self as Iterator>::Item> {
        ...
        if ch == 0x2e { self.index += 1; return self.s1(ctx); }
        if ch == 0x2a { self.index += 1; return self.s3(ctx); }
        if ch == 0x22 { self.index += 1; return self.s4(ctx); }
        if ch == 0x28 { self.index += 1; return self.s8(ctx); }
        if ch == 0x2b { self.index += 1; return self.s9(ctx); }
        if ch == 0x21 { self.index += 1; return self.s10(ctx); }
        ...
        if ch == 0x26 { self.index += 1; return self.s17(ctx); }
        if ch == 0x3f { self.index += 1; return self.s18(ctx); }
        if ch == 0x7c { self.index += 1; return self.s19(ctx); }
        if ch == 0x29 { self.index += 1; return self.s20(ctx); }
        self.begin()
    }
    ...
    fn s1(&mut self, mut ctx: Context) -> Option<<Self as Iterator>::Item> {
        if self.index >= self.input.len() { return self.sink(ctx); }
        let ch = self.input[self.index];
        if ch == 0x2e { self.index += 1; return self.s2(ctx); }
        self.sink(ctx)
    }
    ...
    fn s4(&mut self, mut ctx: Context) -> Option<<Self as Iterator>::Item> {
        ...
        if ch == 0x09 ||
           ...
           (0x23 <= ch && ch <= 0x5b) ||
           (0x5d <= ch && ch <= 0x7e) ||
           (0xa0 <= ch && ch <= 0xff) { self.index += 1; return self.s5(ctx); }
        if ch == 0x5c { self.index += 1; return self.s7(ctx); }
        self.sink(ctx)
    }
    ...
    fn s7(&mut self, mut ctx: Context) -> Option<<Self as Iterator>::Item> {
        ...
        let ch = self.input[self.index];
        if ch == 0x22 ||
           ch == 0x5c { self.index += 1; return self.s5(ctx); }
        self.sink(ctx)
    }
