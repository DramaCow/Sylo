...
// E -> E add T
fn p0(_: EProduct, _: &[u8], _: TProduct) -> EProduct {
    todo!()
}

// E -> T
fn p1(_: TProduct) -> EProduct {
    todo!()
}

// T -> T mul F
fn p2(_: TProduct, _: &[u8], _: FProduct) -> TProduct {
    todo!()
}

// T -> F
fn p3(_: FProduct) -> TProduct {
    todo!()
}

// F -> lparen E rparen
fn p4(_: &[u8], _: EProduct, _: &[u8]) -> FProduct {
    todo!()
}
...
// *** LEXER TABLE START ***
    ...
    fn s0(&mut self, mut ctx: Context) -> Option<<Self as Iterator>::Item> {
        ...
        if (0x61..=0x7a).contains(&ch) { self.index += 1; return self.s2(ctx); }
    ...
    fn s2(&mut self, mut ctx: Context) -> Option<<Self as Iterator>::Item> {
        ...
        let ch = self.input[self.index];
        if (0x61..=0x7a).contains(&ch) { self.index += 1; return self.s2(ctx); }
        Some(Ok(Token { ttype: TokenType::id, span: (ctx.start_index, self.index) }))
    }
...
impl Parse<'_> {
    ...
    fn s0(&mut self) -> Result<(Variable, usize), ParseError> {
        ...
        let tuple = match self.next_token {
            Some(t @ Token { ttype: TokenType::id, .. }) => { self.update()?; self.s5(t) }
    ...
    fn s4(&mut self, arg: Token) -> Result<(Variable, usize), ParseError> {
        ...
        let tuple = match self.next_token {
            Some(t @ Token { ttype: TokenType::id, .. }) => { self.update()?; self.s12(t) }
